...
백업 및 복구 서비스를 이용하려면 먼저 에이전트를 설치해야 합니다. 에이전트를 설치할 서버 정보를 입력한 뒤 설치를 신청하면 담당자가 확인한 뒤 설치를 진행합니다. 에이전트 신청 이력 삭제를 희망할 경우 이용중인 백업/복구 서비스가 모두 중지상태일 때만 이력 삭제가 가능합니다.
1. NHN Cloud 콘솔에서 **Storage > Veritas Backup**을 클릭하세요.
2. **에이전트** 탭에서 **에이전트 설치**를 클릭하세요.
3. **에이전트 설치** 화면에서 에이전트 설치를 위한 정보를 입력한 뒤 **신청**을 클릭하세요.

* **대상**: NHN Cloud의 인스턴스일 경우 선택, VM이나 인스턴스가 아닌 경우 그 외를 선택하세요. 
* **서버**: 에이전트를 설치할 서버를 선택하세요.
* **호스트네임**: 에이전트를 설치할 호스트네임을 입력하세요.
* **신청명**: 신청한 에이전트 설치 요청 건을 식별하기 위한 이름을 입력하세요.
* **연결된 IP**: 인스턴스를 선택한 경우 연결된 서버의 IP 정보가 나타납니다.
* **IP입력**: 에이전트 설치하기 위한 IP정보를 입력하세요. 
* **OS 유형**: 대상이 인스턴스인 경우 선택한 서버의 OS 정보가 나타납니다. 대상이 그 외인 경우 Linux와 Windows 중 알맞은 정보를 선택해주세요.

4. 에이전트 신청 목록에서 해당 신청 정보를 클릭하세요. **신청 정보** 모달 창이 표시됩니다.
5. **에이전트 설치** 행에서 **설치파일 다운로드**를 클릭해 에이전트를 내려받은 뒤 압축을 해제하고 에이전트를 설치하세요. 에이전트 설치가 완료되면 설치 검증을 통해 정상적인 통신 확인 단계를 진행해주세요. 검증이 완료된 경우에만 백업 서비스 신청이 가능합니다. 


## 백업 신청

에이전트가 설치 후 검증이 완료된된 서버만 백업을 신청할 수 있습니다. 신청 후 백업이 완료되면 상세 페이지에서 백업 결과를 조회할 수 있습니다. <br>
백업 결과 리포트를 발행한 뒤 스케줄을 수정한 경우 이전 스케줄의 리포트 이력과 정보는 **스케줄 이력** 탭에서 확인할 수 있습니다. 백업 신청 이력 삭제는 백업 리포트가 존재 시 삭제는 불가능하며 중지만 가능합니다.
...
* 스케줄
  * 첫번째 정보는 풀백업 스케줄 정보를 입력할 수 있으며, 우측 상단의 추가 버튼 클릭시 두번째 풀백업 스케줄 혹은 증분 백업 스케줄 설정이 가능합니다.
  * 스케줄 유형, 스케줄명, 데이터 보관기간, 백업 주기, 시작 시간, 작업 시간을 설정할 수 있습니다.
  * 이전 백업신청 시 이용하였던 스케줄 정보를 재사용해야하는 경우는 템플릿 선택 버튼으로 이전 스케줄 이력을 선택하거나 신규 스케줄 템플릿을 추가할 수 있습니다. 
* 담당자 정보 및 요청사항
...
* 소스 설정
  ...
  * **유형**: 복구할 데이터의 유형을 선택하세요. **파일** 또는 **DB**를 선택할 수 있으며, 유형에 따라 설정 내용이 달라집니다. **파일**의 경우 복구 시점 선택 후 상새새 경로도 복수로 선택할 수 있습니다. **DB**의 경우 **DB 종류**와 **DB명**을 선택한 뒤 복구 시점 선택이 가능합니다.
...
* 타깃 설정
  ...
  * **희망 작업기간**: 복구 유형이 DB인 경우 희망하는 작업 기간을 설정이 필요합니다. 설정한 희망 작업 기간과 실제 서비스 기간과는 차이가 있을 수 있습니다. 
